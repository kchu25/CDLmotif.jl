...
- Julia uses a just-in-time compiler, which means that software written in Julia needs to be pre-compiled before its execution. We hope to get rid of this so that we don't have to wait everytime we launch the code, and so this application can be more easily integrated into the bioinformatics pipeline (e.g., snakemake; see point below).
